...
class RuleBasedClassifier:
    ...
    def _generate_reasoning(
        self, features: Dict[str, float], classification: str, detailed_scores: Dict
    ) -> str:
        ...
        try:
            ...
            strong_features = [
                (name, data) for name, data in feature_breakdown.items() if data["score"] > 0.7
            ]
            ...
            if strong_features:
                ...
                for feature_name, data in strong_features:
                    reasoning_parts.append(
                        f"  - {feature_name}: {data['value']:.3f} (score: {data['score']:.2f})"
                    )
            else:
                reasoning_parts.append(
                    f"Classified as '{classification}' (best match among options)"
                )
            ...
            confidence = detailed_scores.get(classification, {}).get('overall_score', 0.0)
            return f"Classified as '{classification}' with confidence {confidence:.2f}"
